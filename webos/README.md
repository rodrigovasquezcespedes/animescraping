...
## Migración y despliegue en webOS 5

1. Verifica que tienes Enact CLI instalado (`npm install -g @enact/cli`).
2. Instala dependencias: `npm install` en la carpeta webos.
3. Para desarrollo, ejecuta: `npm run serve`.
4. Para empaquetar para webOS: `npm run pack-p`.
5. El archivo `resources/appinfo.json` debe estar presente y configurado.
6. El punto de entrada debe ser `public/index.html` y el div con id `root`.
7. El package.json debe tener el bloque `webos` y dependencias Enact/Sandstone.
8. Usa componentes Sandstone y el patrón `kind` para máxima compatibilidad.
9. Para instalar en un televisor webOS, usa el paquete generado en `dist/`.
